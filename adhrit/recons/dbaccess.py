...
def create_bytecode_table(conn):
	if conn is not None:
		create_table = ''' CREATE TABLE IF NOT EXISTS DataDB(ScanId text PRIMARY KEY NOT NULL, Unsafe_Intent_Urls text, File_Access_Via_Urls text, Content_Access_Via_Urls text, Unencrypted_Socket_Communications text, Insecure_Socket_Factory text, No_Tls_Validity_Checks text, Sticky_Broadcasts text, Empty_Pending_Intents text, Dynamic/exported_Broadcast_Receivers text, Ecb_Instances text, Javascript_Enabled text, Overwritable_Cookie text, Weak_Dynamic_Invocation_Checks_On_Content_Providers text); '''
		try:
			c = conn.cursor()
			c.execute(create_table)
		except Error as e:
